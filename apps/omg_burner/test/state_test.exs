defmodule OMG.Burner.StateTest do
  ...
  def test_token_values(token, accumulated, preexited) do
    assert State.get_accumulated_fees(token) == accumulated
    assert State.get_preexited_fees(token) == preexited
    :ok
  end
